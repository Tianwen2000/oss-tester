...
`performance` suite 会在 1,000 个对象全部处理完后统一输出结果，不会逐对象显示进度，因此终端在上传期间长时间没有新内容通常是正常现象。约 4.39 GiB 数据在 100 Mbps、50 Mbps、20 Mbps 和 10 Mbps 有效上传带宽下，通常分别需要约 6～10 分钟、12～20 分钟、30 分钟和 1 小时；网关限流、请求延迟和重试还会延长时间。一般先等待 10～30 分钟，不要因为暂时没有输出就立即按 `Ctrl+C`。

需要确认进度时，另开一个 SSH 窗口，将下面的示例值完整替换为原窗口实际显示的 `Prefix`，并在末尾加上 `performance/`：

```bash
cd ~/oss-tester
TEST_PREFIX='oss-test:20260901T102325Z-40767f662d:performance/'
python3 oss_cli.py list --prefix "$TEST_PREFIX" --max-keys 1000 | tail -n 1
```

只要 `Objects` 数量持续增加就表示上传仍在进行；达到 `Objects: 1000` 后，原窗口应很快输出最终结果。每次查询也会产生一次列表读请求，不要高频轮询。超过 30 分钟且多次检查的对象数量完全不变时，再排查 Endpoint 限流、请求超时或网络中断。
...
每次下载都会产生一个读请求，并产生约 8 MiB 公网下行流量。终端显示 `Traffic source object` 后即进入 1,152 次下载阶段；单次下载输出被重定向到 `/dev/null`，所以全部 worker 结束前通常不会再显示进度。约 9 GiB 数据在 100 Mbps、50 Mbps、20 Mbps 和 10 Mbps 有效下载带宽下，理论传输时间分别约为 13 分钟、26 分钟、65 分钟和 2.2 小时，实际还要加上 1,152 次 Python 启动、TLS 建连、请求延迟和重试开销。一般至少等待 15～30 分钟，低带宽环境可能需要 1～3 小时。

需要确认任务仍在运行时，可以另开一个 SSH 窗口执行以下只读进程检查；它不会访问 OSS，也不会增加计费请求：

```bash
pgrep -af 'oss_cli.py.*download'
```

能够看到下载进程表示 worker 仍在工作；进程偶尔恰好处于两次下载的切换间隙时结果可能为空，可间隔几秒再查一次。不要仅因为终端暂时没有输出就按 `Ctrl+C`。只有最后显示 `PASS: downloaded 1152 times` 才代表全部 worker 成功；后台任务编号或提前打印的对象 Key 都不代表完成。如果通过 CDN 域名下载，计量分类可能变为 CDN 回源或 CDN 下行，必须结合产品计费规则和日志核对。流量统计更新后，按输出的 `Prefix` 清理源对象。
